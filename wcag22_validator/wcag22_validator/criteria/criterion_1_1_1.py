...
class Criterion_1_1_1(BaseCriterion):
    ...
    def _generate_img_alt_solution(self, img: Tag, decorative: bool = None) -> str:
        ...
        attrs_dict = {k: v for k, v in img.attrs.items() if k != 'alt'}
        
        if decorative:
            attrs_dict['alt'] = ''
            if 'role' not in attrs_dict or attrs_dict['role'].lower() not in ['presentation', 'none']:
                 attrs_dict['role'] = 'presentation'
            attrs_str = self._build_attrs_string(attrs_dict) # Using the helper
            return f'<img {attrs_str}>'
        else:
            attrs_str = self._build_attrs_string(attrs_dict)
            return f'<img {attrs_str} alt="[Descriptive text about the image]">'
    ...
    def _generate_longdesc_solution(self, img: Tag) -> str:
        ...
        Returns:
            HTML string with solution
        """
        attrs_dict = {k: v for k, v in img.attrs.items() if k not in ['alt', 'aria-describedby']}
        attrs_str = self._build_attrs_string(attrs_dict) # Using the helper
        alt_text = img.get('alt', '[Brief description]')
        if len(alt_text) > 50:
            alt_text = "[Brief, concise alt text]"

        # Create a unique ID for aria-describedby if needed
        fig_id = f"img-desc-{img.sourceline or hash(str(img))%10000}"

        return f"""<figure>
  <img {attrs_str} alt=\"{alt_text}\" aria-describedby=\"{fig_id}\">
  <figcaption id=\"{fig_id}\">[Detailed description of the image]</figcaption>
</figure>"""
    ...
    def _generate_svg_solution(self, svg: Tag) -> str:
        ...
        Returns:
            HTML string with solution
        """
        svg_inner_content = svg.decode_contents() 
        svg_inner_display = svg_inner_content[:100] + '...' if len(svg_inner_content) > 100 else svg_inner_content
        
        svg_attrs_dict = {k:v for k,v in svg.attrs.items() if k not in ['aria-label', 'role', 'aria-labelledby']}
        svg_attrs_str = self._build_attrs_string(svg_attrs_dict) # Using the helper

        return f"""<svg {svg_attrs_str} aria-label="[Descriptive text for SVG]" role="img">
  <title>[Accessible Name via Title]</title>
  {svg_inner_display}
</svg>
<!-- Or, if using aria-labelledby: -->
<!-- <span id=\"svg-label-id\">[Descriptive text]</span> -->
<!-- <svg {svg_attrs_str} aria-labelledby=\"svg-label-id\" role=\"img\"> -->
<!--  {svg_inner_display} -->
<!-- </svg> -->"""
    ...
    def _generate_area_solution(self, area: Tag) -> str:
        ...
        Returns:
            HTML string with solution
        """
        attrs_dict = {k: v for k, v in area.attrs.items() if k != 'alt'}
        attrs_str = self._build_attrs_string(attrs_dict) # Using the helper
        return f'<area {attrs_str} alt="[Description of link destination]">'
    ...
    def _generate_input_image_solution(self, input_img: Tag) -> str:
        ...
        Returns:
            HTML string with solution
        """
        attrs_dict = {k: v for k, v in input_img.attrs.items() if k != 'alt'}
        attrs_str = self._build_attrs_string(attrs_dict) # Using the helper
        return f'<input {attrs_str} alt="[Description of button function]">'
    ...
    def _generate_canvas_solution(self, canvas: Tag) -> str:
        ...
        Returns:
            HTML string with solution
        """
        attrs_dict = {k: v for k, v in canvas.attrs.items() if k != 'aria-label'}
        attrs_str = self._build_attrs_string(attrs_dict) # Using the helper
        return f"""<canvas {attrs_str} aria-label="[Accessible name for canvas content]">
  <!-- Fallback Content -->
  <p>Your browser does not support the canvas element. This canvas displays [description of content].</p>
</canvas>"""
